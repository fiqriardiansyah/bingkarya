...
export interface MusicInfoWithFile extends MusicInfo {
    type: UploadType;
    komersil: boolean;
}

type Props = { onUpload?: (value: MusicInfoWithFile) => void; loading?: boolean };
...
export default function Sidebar({ onUpload, loading }: Props) {
    ...
    const onSubmitInfoAlbum = (values: any) => {
        if (onUpload) onUpload({ ...values, type, komersil });
    };

    return (
        <Card className="h-fit">
            <CardHeader>
                <CardTitle>Upload Karya Music</CardTitle>
            </CardHeader>
            <CardContent>
