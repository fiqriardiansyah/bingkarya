...
import TaskUploader from "@/service/uploader";
...
const useFileUpload = (uploader: (params: UploaderProps) => UploadTask, { cid }: Pick<IDs, "cid">) => {
    ...
    const uploadFile = (prm: UploaderProps) => {
        ...
        const taskUploader = new TaskUploader(
            prm.messageId,
            uploadTask,
            (s) => onSnapshot(s, prm.messageId),
            (e) => onError(e, prm.messageId),
            () => onComplete(prm.messageId, uploadTask)
        );
        addTask({
            mid: prm.messageId,
            error: null,
