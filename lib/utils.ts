import { RcFile } from "antd/es/upload";
import { type ClassValue, clsx } from "clsx";
import moment from "moment";
import { twMerge } from "tailwind-merge";
import { v4 as uuid } from "uuid";

export function cn(...inputs: ClassValue[]) {
    return twMerge(clsx(inputs));
}

export function cleanObj(obj: Object) {
    return Object.entries(obj).reduce((a: any, [k, v]) => (v ? ((a[k] = v), a) : a), {});
}

export function stripHtml(html: string) {
    const tmp = document.createElement("DIV");
    tmp.innerHTML = html;
    return tmp.textContent || tmp.innerText || "";
}

export function parseTreeObjectToArray<T>(obj: any): T[] {
    if (!obj) return [];
    const parse = Object.keys(obj).map((key) => ({
        ...obj[key],
        id: key,
    })) as T[];
    return parse as T[];
}

export function removeDashes(str: string) {
    return str.split("-").join("");
}

export function createChatId({ postfix, uids }: { uids: string[]; postfix: string }) {
    return removeDashes(uids.sort().join("") + postfix);
}

export function generateFileName({ file }: { file: File }) {
    const extension = file.name.split(".")[file.name.split(".").length - 1].toLowerCase();
    return `${uuid()}.${extension}`;
}

export function onPaste(e: any): Promise<any> | null {
    if (!(e.clipboardData && e.clipboardData.items)) return null;
    return new Promise((resolve, reject) => {
        for (let i = 0, len = e.clipboardData.items.length; i < len; i++) {
            const item = e.clipboardData.items[i];
            if (item.kind === "string") {
                item.getAsString((str: any) => resolve({ type: "text", value: str }));
            } else if (item.kind === "file") {
                const pasteFile = item.getAsFile();
                resolve({ type: "file", value: pasteFile });
            } else {
                reject(new Error("Not allow to paste this type!"));
            }
        }
    });
}

export function getBase64(data: RcFile | File, callback: (url: string) => void) {
    const reader = new FileReader();
    reader.addEventListener("load", () => callback(reader.result as string));
    reader.readAsDataURL(data);
}

export function isDifferentDate({ prevTime, time }: { prevTime: any; time: any }) {
    return moment(prevTime || moment.now()).isSame(time, "date");
}

export const FORMAT_DATE = "DD MMM YYYY, HH:mm";
export const IMAGES_TYPE = ["image/png", "image/jpg", "image/jpeg", "image/webp"];
export const IMAGE_FALLBACK = `data:image/png;base64,`;
export const DEFAULT_ERROR = "Ooops something went wrong!";
export const FILE_TYPE_ERROR = "file type not support!";
export const FILE_SIZE_ERROR = "file size too big or too small!";
